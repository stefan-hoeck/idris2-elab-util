...
by using some predicates. In previous parts of this tutorial, I have shown in
detail, how such top-level declarations can be generated by means of
elaborator reflection via a generic representation of data types.
...
interface for an indexed type. We so far typically only accepted
...
`Type -> Type` such as `Eq` or `Semigroup`. I'm going to
somewhat loosen this restriction in the course of this chapter and
show, how to derive implementations for certain indexed types.
...
compare all arguments of matching constructors, and terminate
...
require much thought. Here is a slightly more interesting example:
...
We interpret boolean equality mainly as a runtime check, so the produced
code just ignores the erased field. In the above case, this works fine,
but in the general case, it doesn't. Lets have a look at another sigma
type:
...
Things get a lot trickier once we start deriving implementations for
...
%runElab derive "Const"
  [ FromDouble, Show, Eq, Ord, Semigroup, Monoid, Num, Abs, Neg
  , Integral, Fractional, FromString, FromChar]
```
...
twelve = 20 - 8
```
Doc.Derive> :t implNumConst
Doc.Derive.implNumConst : Num c => Num (Const c a)
...
%runElab derive "Barbie" [Show, Eq, Semigroup, Monoid]

barbie : Barbie (Prelude.id) Integer
barbie = MkBarbie 1 "Stefan" 30

listBarbie : Barbie List Void
listBarbie = MkBarbie [] ["Foo"] [2, 20, 80]

testBarbie : (Derive.barbie == Derive.barbie) === True
testBarbie = Refl

testEmptyBarbie : (Derive.listBarbie <+> Prelude.neutral) = Derive.listBarbie
testEmptyBarbie = Refl
```
